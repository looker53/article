...
那么一个web框架，首先应该具备一些什么样的核心功能呢？ 我觉主要有3个：接收用户请求、提供开发者接口、响应结果。
...
当 handle 方法处理一个实际的请求时，

```python
class BaseHandler:    
    def handler(self, socket: socket.socket):
        with socket:
            try:
                request = Request.from_socket(socket)
            except:
                ...
                for router in self.routers:
                    if router.url == request.url:
                        response = router.view_func(request)
                        response.send(socket)
                else:
